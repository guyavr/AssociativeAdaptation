...
% and compound conditioning experiments. (Figs. 1E, 1F, and 2C)
clc
close all
clear
...
sameSchedule=0; % 0- different schedule for each participant (within a simulation)
                % 1- same schedule all participants (within a simulation)
                % 2- same schedule for all simulations and participants
                % (load a saved schedule)
sameVaryingParameters=0; % 1- load saved parameters

% models parameters
% state space
A = 0.9; % retention
B = 0.15; % learning rate
err = 15; % sensory prediction error (it's a clamp, but we are going to treat it as rotation, i.e., an internal SPE that decreases as adaptation proceeds
...
for exp= 1:2 % 1- differential; 2- compound
    ...
    if exp == 1
        ...
        paramVal_RW=[0.99 0.002 0.002 0.12 15]; % exp 1 - differential- testing some variations THE ONE THAT WAS USED!
        ...
        paramVal_RW=[0.99 0.1 0.1 0.02 15]; % exp 2 - compound
...
%             us = [Shuffle(tmp_us) zeros(1,nProbe)];
...
%             probe_cs = Shuffle(tmp_probe); % shuffle
...
for exp=1:2
    if exp==1
        ...
        plotDiffHandAngle_Differential_sim_current(dha_acq_RW)
        plotDiffHandAngle_Differential_sim_current(dha_probe_RW)
        
    else
        
        if newSet_exp2
            save('simSet_Exp2_Comp','dha_SS','dha_RW')
        else
            load('simSet_Exp2_Comp')
        end
        
        plotDiffHandAngle_CompoundToneLight_sim_current(dha_RW)
