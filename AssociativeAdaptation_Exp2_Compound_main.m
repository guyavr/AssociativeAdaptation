...
ha = removeOutlierTrials(ha_cw,wash_i);
...
% Movement Time
mt=reshape(T.MT,nT,[])';
mt_thresh=0.3; % "move faster" message in the experiment (THINK OF WHETHER TO INCLUDE THIS)
...
cs_tone=reshape(T.cs_tone,nT,[])';
cs_light=reshape(T.cs_frame,nT,[])';

comp.ha=ha;
comp.dha=dha;
comp.cs_tone=cs_tone;
comp.cs_light=cs_light;
comp.rt=rt;
...
% Plot mean time courses across participants (Fig. S1B)
plotMeanTimeCourse_Compound(ha_timecourse,tr_wash)

% Bar plots- summary analysis - Change in Hand Angle (Figs. 3B, 3D)
...
% plotBarsMeanDiffHandAngle_Compound_Indiv(Conditioning.dHA,1,t_n1)
